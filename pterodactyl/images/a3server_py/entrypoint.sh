...
then
	...
	for i in $(seq ${HC_NUM})
	do
		./${SERVER_BINARY} -client -connect=${HC_PORT} -port=${SERVER_PORT} -password="${HC_PASSWORD}" -profiles=./serverprofile -bepath=./battleye -mod="${MODIFICATIONS}" ${STARTUP_PARAMS} > /dev/null 2>&1 &
